...
@Component({
  selector: 'appcolp-sub-mas-informacion',
  templateUrl: './sub-mas-informacion.component.html',
  styleUrls: ['./sub-mas-informacion.component.css']
})
export class SubMasInformacionComponent implements OnInit {

  listaCategorias: SubCategoriaMasInfo[] = null;
  categoria: SubCategoriaMasInfo = null;

  categoriaPadre: CategoriaMasInformacion = null;

  tipoFormulario: number = null;
  ...
  constructor(
    ...
    this.route
      .queryParamMap
      .subscribe(e => {
        this.idCategoriaPadre = parseInt(e.get('opc')); // parametro opc es el id de la alianza
      });
   }
  ...
  listarCategorias(){
    this.service.getSub(this.idCategoriaPadre).subscribe(res=>{
  ...
  abrirModalAccion(accion, id) {
    ...
    if (accion == "guardar") {
      this.categoria = new SubCategoriaMasInfo();
  ...
  cambiarEstado(event, id: number) {
    ...
    this.service.estadoSub(this.categoria).subscribe(res => {
  ...
  eliminar() {
    ...
    this.service.eliminarSub(this.categoria).subscribe(res => {
      let mensaje = "Se elimino la categoria exitosamente";
  ...
  guardarCambios(){
    ...
    if (this.categoria.titulo == '' || this.categoria.titulo == null || this.categoria.titulo == undefined) {
      this.campos = true;
    ...
    if(this.campos == false){
      if(this.accion == "guardar"){
        this.service.guardarSub(this.categoria).subscribe(res=>{
        ...
        this.service.actualizarSub(this.categoria).subscribe(res=>{
          let mensaje = 'Su contenido se ha guardado exitosamente'
          if(this.enviarPUSH) this.enviarPush()
          else this.eliminarConfirma(mensaje, "guardar");
          console.log(res)
