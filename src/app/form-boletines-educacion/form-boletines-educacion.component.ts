...
import { BoletinesEducacionService  } from '../servicios/boletines-educacion.service';    
import { BoletinesBienestar } from '../modelo/boletines-bienestar';

@Component({
  selector: 'appcolp-form-boletines-educacion',
  templateUrl: './form-boletines-educacion.component.html',
  styleUrls: ['./form-boletines-educacion.component.css']
})
export class FormBoletinesEducacionComponent implements OnInit {
  ...
  constructor(
    public service: BoletinesEducacionService,
      ...
      if (this.validacionFechas == true) {
        this.service.guardarCambios(this.boletin, this.accion).subscribe(res => {
          ...
          if (this.enviarPUSH == true) {
            ...
            if (this.accion == 'editar') tituloMensajePush = "Sección Bienestar: Se modifico un boletín"
            let envio = {
  ...
  anadirImagen(){
    ...
    if(this.accion == 'editar'){
      let type = 'image/png';
      const byteCharacters = atob(this.boletin.imagen);
      ...
      const imageFile: File = new File([blob], this.boletin.titulo, {
        type: "image/png"
      });
  ...
  atras() {
    this.router.navigateByUrl('seccionEducacion', { skipLocationChange: true }).then(() => this.router.navigate(['seccionEducacion/boletines']));
  }
}
